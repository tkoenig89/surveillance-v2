...
#remove old images after 3 days(4000 min) 
(crontab -l 2>/dev/null; echo "1 * * * * $CLIENTSCRIPTS_FOLDER/cleanupImageFolder.sh $IMAGE_BASEFOLDER 4000") | crontab -

#append jobs to shoot images
(crontab -l 2>/dev/null; echo "* * * * * $CLIENTSCRIPTS_FOLDER/shootImage.sh $IMAGE_BASEFOLDER") | crontab -
